...
function InternationalDelegates() {
  ...
  const addToCart = async (eventId) => {
    ...
    try {
      ...
      if (addToCartResponse.ok) {
        ...
        if(eventId===29){
        const updateBRStatusResponse = await fetch('https://api.jwcmedicalolympics.com/api/update-br-status', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  return (
    ...
    <Link to="https://drive.google.com/file/d/1zT3QRNlinSK63RmQyigN14VFDxa8mqWb/view?usp=drivesdk" className="itinerarylink">Check out our International Delegates Brochure</Link>
      <header>
