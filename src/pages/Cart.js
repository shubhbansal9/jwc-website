        ...
        if (userProfile && userProfile.email) {
          ...
          if (cartData.success && cartData.cart) {
            ...
            const eventDetailsPromises = eventIds.map(async (eventId) => {
              const eventResponse = await fetch(`http://64.227.156.132:3001/api/events/${eventId}`);
        ...
        {eventsDetails.map((event, index) => (
          <li key={index} className="cart-item">
            ...
            <button onClick={async () => {
              try {
                const response = await fetch(`http://64.227.156.132:3001/api/remove-from-cart/${userProfile.email}/${event.eventId}`, {
                  method: 'POST',
                });
